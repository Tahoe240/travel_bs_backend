    ...
    <div id="footer-outer">
      <div class="footer-content">
        <div class="item contact-me">
          <h3>联系我</h3>
          <p>Email:2407939417@qq.com</p>
          <p>Phone:15980426833</p>
        </div>
  ...
  export default {
    ...
    methods: {
      ...
      handleCommand(command) {
        ...
        if (command === "logout") {
          axios.get("/user/toLogout").then(res => {
    ...
    created() {
      console.log("index===>头像链接："+this.$store.state.login_user.avatar);
      let login_user = JSON.parse(localStorage.getItem('login_user'));

      console.log("index===>本地存储头像链接"+login_user.avatar);
      this.$store.commit("updateLoginUser",login_user);
      console.log("index===>头像链接："+this.$store.state.login_user.avatar);
