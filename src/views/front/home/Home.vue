	...
	export default {
		...
		methods:{
			initTravelNoteList(){
				...
				let baseUrl = "/api/travelNote/getByTitleType?" + this.$qs.stringify(params,{encode:false});
          ...
          headers:{
            Authorization: `Bearer ${localStorage.getItem('token')}`
					...
					if(res.data.code=== 200){
						...
						this.travelNoteList.push(...res.data.data.records);
			...
			initCarousel(){
				//按照点赞数获取6个轮播数据
				axios.get("/api/travelNote/getByUpDesc",
            {
              headers:{
                Authorization: `Bearer ${localStorage.getItem('token')}`
              }
					...
					if(res.data.code=== 200){
						...
						this.carouselImages.push(...res.data.data);
					}
				})
			},

			initTravellerNoteList(){
				axios.get("/api/travelNote/getByTime?limit=4",{
          headers:{
            Authorization: `Bearer ${localStorage.getItem('token')}`
          }
        }).then(res =>{
					...
					if(res.data.code=== 200){
						...
						this.TravellerNoteList.push(...res.data.data);
					}
				})
			},
      ...
      console.log("头像链接："+this.$store.state.login_user.avatar)
		},
		watch:{
			//监测名-----精简写法
